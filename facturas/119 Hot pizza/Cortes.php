...
public function CortePrint($data, $printer){
    ...
    $printer->feed();
    $printer->feed();
    $printer -> text("APERTURA: ".$data['apertura']);
    $printer->feed();
    $printer -> text("CIERRE: ".$data['cierre']);
    ...
    $printer -> text($doc->DosCol("TOTAL TRANFERENCIAS: ", 30, Helpers::Dinero($data['transferencias']), 10));
