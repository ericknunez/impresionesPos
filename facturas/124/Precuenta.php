...
public function PrecuentaPrint($data, $printer){
    ...
    $img  = "C:/Appserv/www/impresiones/facturas/124/img/logo.png";
  ...
  //   $printer->text($data['empresa_nombre']);
    $printer->feed(2);
    //$printer->text("EL CORRALITO");
    $printer -> setFont(Printer::FONT_A); 
    $printer -> setTextSize(1, 1);
    $printer -> setLineSpacing(80);
    $printer->feed();
    $printer->text("Dirección: 3° Calle Ote. Casa N° 6 ");
    $printer->feed();
  
    $printer->text("Barrio Dolores, Izalco, Sonsonate");
    $printer->feed();
    $printer->text("TELEFONO: 7139-5388");
  $printer->feed();
